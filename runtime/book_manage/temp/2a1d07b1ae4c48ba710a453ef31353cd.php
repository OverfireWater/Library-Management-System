<?php /*a:1:{s:82:"D:\phpstudy_pro\WWW\Library-Management-System\app\book_manage\view\user\index.html";i:1685606630;}*/ ?>
﻿<!DOCTYPE html>
<html class="no-js">
    ...
    <div class="flex flex-grow items-center lg:w-3/4 xl:w-4/5 coniant">
        <form action="userView" method="get" id="search_form">
			<span class="relative w-full" style="width: 300px;">
				<input type="search" placeholder="搜索" name="keywords" value="<?php echo htmlentities($keywords); ?>" autocomplete="off" id="search"
                       style="color: white"
...
<div class="flex">
        ...
        <ul class="list-reset mb-8 w-full">
            ...
            <li class="ml-2 mb-4">
                ...
                <ul class="text-grey lg:text-grey-dark list-reset leading-loose mt-2" id="sidenav-categories">
                    <?php foreach($bookType as $val): ?>
                    <li class="hover:text-indigo-dark hover:cursor-pointer transition-normal ml-1 border-l border-grey-dark pl-4">
                        <a href="?keywords=<?php echo htmlentities($val['BTName']); ?>"><?php echo htmlentities($val['BTName']); ?></a>
                    </li>
                    <?php endforeach; ?>
                </ul>
            </li>
    ...
    <div class="flex flex-1 flex-col md:px-6 pt-10" id="bookInfo">
        ...
        <div class="hidden px-2 pt-2 md:px-0 flex-wrap order-2 pb-8 js-tab-pane active" id="section-library">
            <?php if(($bookInfo)): foreach($bookInfo as $val): ?>
            <div class="flex flex-row sm:flex-col items-center sm:items-start w-full xs:w-1/2 sm:w-1/3 md:w-1/5 p-4 js-book">
                <a href="details?b_id=<?php echo htmlentities($val['id']); ?>&&state=<?php echo htmlentities($val['BState']); ?>&&borrow=0">
                    <img src="<?php echo htmlentities($val['BUrl']); ?>" alt="book-01" style="width: 141px;height: 180px"
                    ...
                    <div class="ml-3 sm:ml-0 w-2/3 sm:w-full">
                        <p class="text-sm my-2 font-medium sm:font-normal" style="height:1.2rem;overflow: hidden">
                            <?php echo htmlentities($val['BName']); ?></p>
                        <label for=""
                               class="hidden sm:inline-block rounded-full text-white px-2 py-1/2 text-xs
                           <?php if(($val['BState']==0)): ?> libre-bg-grey
                        <?php elseif(($val['BState']==1)): ?> bg-indigo-medium
                    <?php elseif(($val['BState']==2)): ?> libre-bg-yellow
                    <?php elseif(($val['BState']==3)): ?> yellow
                    <?php else: ?> red
                    <?php endif; ?>
                    ">
                            <?php if(($val['BState']==0)): ?> <span>仓库</span>
                            <?php elseif(($val['BState']==1)): ?> <span >上架</span>
                            <?php elseif(($val['BState']==2)): ?><span>外借</span>
                            <?php elseif(($val['BState']==3)): ?><span>丢失</span>
                            <?php else: ?><span style="color: #e5e1d6">报废</span>
                            <?php endif; ?></label>
                    </div>
                </a>
            </div>
            <?php endforeach; else: ?> <span>暂无图书</span>
            <?php endif; ?>
        </div>
    ...
    <div class="hidden flex-1 flex-col md:px-6 pt-10" id="myborrow_book">
        ...
        <!-- Library -->
        <div class="hidden px-2 pt-2 md:px-0 flex-wrap order-2 pb-8 js-tab-pane active" id="section-library">
            <?php if((is_array($myborrow_book))): foreach($myborrow_book as $val): ?>
            <div class="flex flex-row sm:flex-col items-center sm:items-start w-full xs:w-1/2 sm:w-1/3 md:w-1/5 p-4 js-book">
                <a href="details?b_id=<?php echo htmlentities($val['bookinfo']['id']); ?>&&state=<?php echo htmlentities($val['bookinfo']['BState']); ?>&&borrow=1">
                    <img src="<?php echo htmlentities($val['bookinfo']['BUrl']); ?>" alt="book-01" style="width: 141px;height: 180px"
                         class="w-1/3 sm:w-full shadow-md transition-normal hover:brighter hover:translate-y-1 hover:shadow-lg hover:border-indigo">
                    <div class="ml-3 sm:ml-0 w-2/3 sm:w-full">
                        <p class="text-sm my-2 font-medium sm:font-normal" style="height:1.2rem;overflow: hidden"><?php echo htmlentities($val['bookinfo']['BName']); ?></p>
                        <label for=""
                               class="hidden sm:inline-block rounded-full text-white px-2 py-1/2 text-xs "
            ...
            <?php endforeach; else: ?> <span><?php echo htmlentities($myborrow_book); ?></span>
            <?php endif; ?>
        </div>
    </div>
    <!-- 个人中心 -->
            ...
            <form class="layui-form" id="form_data" action="" style="width: 30rem;margin-top: 30px">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="name" value="<?php echo htmlentities($name); ?>" required lay-verify="required"
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="phone" required value="<?php echo htmlentities($phone); ?>" lay-verify="required"
    ...
    <div class="hidden fie absolute pin-b z-10 lg:relative lg:block w-full lg:w-1/5 bg-grey-lighter-2 px-6 pt-10"
         id="profile"
         <?php if(($mybook_count==0)): ?> style="height: 500px;"
         <?php elseif(($mybook_count>=6)): ?> ""
        <?php endif; ?>
        >
        ...
        <div class="my-4 border-t pt-4">
            <h3 class="text-indigo-dark font-normal">你借阅了<strong><?php echo htmlentities($mybook_count); ?></strong>本图书</h3>
            <div class="flex flex-wrap -ml-2 justify-start items-center">
                <?php if((is_array($myborrow_book))): foreach($myborrow_book as $val): ?>
                <img src="<?php echo htmlentities($val['bookinfo']['BUrl']); ?>" width="58" height="68" alt="read"
                     class="w-1/6 lg:w-1/5 max-w-tiny shadow-md block m-2 transition-normal hover:brighter">
                <?php endforeach; else: ?> <span style="margin-top: 20px">还没有图书被借阅哦</span>
                <?php endif; ?>
            </div>
        </div>
...
<script>
    $(function () {
        $("#book_state option[value='<?php echo htmlentities($state); ?>']").prop("selected","selected");
    ...
    layui.use(['form', 'layer'], function () {
        ...
        form.on('select(select-filter)',function (data){
            if (data.value!=""){
                ...
                url="?keywords=<?php echo htmlentities($keywords); ?>&state="+value
